...
static size_t furi_hal_serial_dma_bytes_available(FuriHalSerialId ch) {
    ...
    if((furi_hal_serial[ch]->dma_rx_node_half_buf && (dma_remain != 0)) ||
       (!furi_hal_serial[ch]->dma_rx_node_half_buf && (dma_remain == 0))) {
...
static void furi_hal_serial_irq(void* context) {
    ...
    if(event) {
        size_t len = furi_hal_serial_dma_bytes_available(handle->id);
...
static void furi_hal_serial_dma_rx_irq(void* context) {
    ...
    if(LL_DMA_IsActiveFlag_TC(GPDMA1, furi_hal_serial[handle->id]->dma_rx_channel) != 0U) {
        ...
        size_t len_tc = furi_hal_serial_dma_bytes_available(handle->id);
    ...
    if(LL_DMA_IsActiveFlag_HT(GPDMA1, furi_hal_serial[handle->id]->dma_rx_channel) != 0U) {
        ...
        size_t len_ht = furi_hal_serial_dma_bytes_available(handle->id);
...
bool furi_hal_serial_init(FuriHalSerialHandle* handle, uint32_t baud) {
    ...
    furi_hal_interrupt_set_isr_ex(
        furi_hal_serial_config[handle->id].irq,
...
void furi_hal_serial_deinit(FuriHalSerialHandle* handle) {
    ...
    furi_hal_interrupt_set_isr(furi_hal_serial_config[handle->id].irq, NULL, NULL);
...
void furi_hal_serial_get_rx_data_flush(FuriHalSerialHandle* handle) {
    furi_hal_serial_check(handle);
    furi_hal_serial[handle->id]->rx_dma_buf_index_read =
        furi_hal_serial[handle->id]->rx_dma_buf_index_write;
    furi_hal_serial[handle->id]->rx_dma_buf_count_bytes = 0;
}
...
void furi_hal_serial_set_rx_dma_buffer_size(FuriHalSerialHandle* handle, uint16_t size) {
    ...
    LL_DMA_DeInit(GPDMA1, furi_hal_serial[handle->id]->dma_rx_channel);
    ...
    furi_hal_serial[handle->id]->rx_dma_buf_index_write = 0;
    furi_hal_serial[handle->id]->rx_dma_buf_index_read = 0;
    furi_hal_serial[handle->id]->rx_dma_buf_count_bytes = 0;
    furi_hal_serial[handle->id]->dma_rx_node_half_buf = false;
