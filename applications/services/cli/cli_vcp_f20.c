...
#define USB_CDC_PKT_LEN CDC_MAX_PACKET_LEN
...
static void vcp_on_cdc_control_line(bool dtr, bool rts, void* context);

static CdcCallbacks cdc_cb = {
    .rx_callback = vcp_on_cdc_rx,
    .tx_done_callback = vcp_on_cdc_tx_complete,
...
static int32_t vcp_worker(void* context) {
    ...
    //ToDo add loading old configuration
    ...
    furi_hal_usb_set_config(&usb_default, &cdc_cfg); // TODO: move to usb init
    ...
    while(1) {
        ...
        if(flags & VcpEvtTx) {
            ...
            if(len > 0) { // Some data left in Tx buffer. Sending it now
                tx_idle = false;
                furi_hal_cdc_write(vcp->data_buffer, len);
