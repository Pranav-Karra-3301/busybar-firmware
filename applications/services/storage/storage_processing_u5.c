...
static StorageType storage_get_type_by_path(FuriString *path) {
  ...
  if (memcmp(path_cstr, STORAGE_EXT_PATH_PREFIX,
             strlen(STORAGE_EXT_PATH_PREFIX)) == 0) {
    type = ST_EXT;
  } else if (memcmp(path_cstr, STORAGE_INT_PATH_PREFIX,
                    strlen(STORAGE_INT_PATH_PREFIX)) == 0) {
    type = ST_INT;
  } else if (memcmp(path_cstr, STORAGE_ANY_PATH_PREFIX,
                    strlen(STORAGE_ANY_PATH_PREFIX)) == 0) {
    type = ST_ANY;
  }
...
static FS_Error storage_get_data(Storage *app, FuriString *path,
                                 StorageData **storage) {
  ...
  if (storage_type_is_valid(type)) {
    ...
    if (type == ST_ANY) {
      FURI_LOG_W(TAG, STORAGE_ANY_PATH_PREFIX
                 " is deprecated, use " STORAGE_EXT_PATH_PREFIX " instead");
...
bool storage_process_file_open(Storage *app, File *file, FuriString *path,
  ...
  if (file->error_id == FSE_OK) {
    if (storage_path_already_open(path, storage)) {
      ...
      FS_CALL(storage, file.open(storage, file, path_cstr_no_vfs, access_mode,
                                 open_mode));
    }
  }
...
static uint16_t storage_process_file_write(Storage *app, File *file,
                                           const void *buff,
...
static bool storage_process_file_seek(Storage *app, File *file,
...
bool storage_process_dir_open(Storage *app, File *file, FuriString *path) {
  ...
  if (file->error_id == FSE_OK) {
    if (storage_path_already_open(path, storage)) {
      ...
      storage_push_storage_file(file, path, storage);
      FS_CALL(storage,
              dir.open(storage, file, cstr_path_without_vfs_prefix(path)));
    }
  }
...
bool storage_process_dir_read(Storage *app, File *file, FileInfo *fileinfo,
                              char *name, const uint16_t name_length) {
  bool ret = false;
  StorageData *storage = get_storage_by_file(file, app->storage);
...
static FS_Error storage_process_common_stat(Storage *app, FuriString *path,
                                            FileInfo *fileinfo) {
  StorageData *storage;
  FS_Error ret = storage_get_data(app, path, &storage);
  ...
  if (ret == FSE_OK) {
    FS_CALL(storage,
            common.stat(storage, cstr_path_without_vfs_prefix(path), fileinfo));
  }
...
static FS_Error storage_process_common_remove(Storage *app, FuriString *path) {
  ...
  do {
    ...
    storage_data_timestamp(storage);
    FS_CALL(storage,
            common.remove(storage, cstr_path_without_vfs_prefix(path)));
  } while (false);
...
static FS_Error storage_process_common_fs_info(Storage *app, FuriString *path,
  ...
  if (ret == FSE_OK) {
    FS_CALL(storage, common.fs_info(storage, cstr_path_without_vfs_prefix(path),
...
static bool storage_process_common_equivalent_path(Storage *app,
                                                   FuriString *path1,
                                                   FuriString *path2) {
  bool ret = false;
  ...
  do {
    ...
    FS_CALL(storage, common.equivalent_path(furi_string_get_cstr(path1),
                                            furi_string_get_cstr(path2)));
...
void storage_process_alias(Storage *app, FuriString *path,
                           FuriThreadId thread_id, bool create_folders) {
  if (furi_string_start_with(path, STORAGE_APP_DATA_PATH_PREFIX)) {
    FuriString *apps_data_path_with_appsid =
        furi_string_alloc_set(APPS_DATA_PATH "/");
    ...
    if (create_folders &&
        storage_process_common_stat(app, apps_data_path_with_appsid, NULL) !=
            FSE_OK) {
    ...
    FuriString *apps_assets_path_with_appsid =
        furi_string_alloc_set(APPS_ASSETS_PATH "/");
    furi_string_cat(apps_assets_path_with_appsid,
                    furi_thread_get_appid(thread_id));
    ...
    FuriString *int_on_ext_path =
        furi_string_alloc_set(EXT_PATH(STORAGE_INTERNAL_DIR_NAME));
...
void storage_sd_presence_changed(Storage *app) {
  ...
  if (app->storage[ST_EXT].status == StorageStatusNotReady &&
      app->sd_alive == true) {
    app->sd_alive = false;
...
void storage_process_message_internal(Storage *app, StorageMessage *message) {
  ...
  case StorageCommandFileEof:
    message->return_data->bool_value =
        storage_process_file_eof(app, message->data->file.file);
    break;
  ...
  case StorageCommandDirRead:
    message->return_data->bool_value = storage_process_dir_read(
        app, message->data->dread.file, message->data->dread.fileinfo,
  ...
  case StorageCommandCommonTimestamp:
    ...
    storage_process_alias(app, path, message->data->ctimestamp.thread_id,
                          false);
    message->return_data->error_value = storage_process_common_timestamp(
        app, path, message->data->ctimestamp.timestamp);
    break;
  ...
  case StorageCommandCommonRemove:
    ...
    message->return_data->error_value =
        storage_process_common_remove(app, path);
    break;
  ...
  case StorageCommandCommonFSInfo:
    ...
    message->return_data->error_value = storage_process_common_fs_info(
        app, path, message->data->cfsinfo.total_space,
  ...
  case StorageCommandCommonEquivalentPath: {
    ...
    storage_process_alias(app, path1, message->data->cequivpath.thread_id,
                          false);
    storage_process_alias(app, path2, message->data->cequivpath.thread_id,
                          false);
