...
static void wifi_rf_test_command_print_usage(void) {
    printf("Usage:\r\n");
    printf("wifi_rf_test \"WiFi rf test app\"\r\n");
}

static void wifi_rf_test_command(Cli* cli, FuriString* args, void* context) {
    FuriString* cmd;
    cmd = furi_string_alloc();

    do {
        if(!args_read_string_and_trim(args, cmd)) {
            wifi_rf_test_command_start(cli, args, context);
            break;
        }

        wifi_rf_test_command_print_usage();
    } while(false);

    furi_string_free(cmd);
}

void wifi_rf_test_system_start(void) {
#ifdef SRV_CLI
