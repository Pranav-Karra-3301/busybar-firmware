...
static const sl_wifi_device_configuration_t sl_wifi_default_enterprise_client_configuration_cli = {
    ...
    .region_code = US,
    .boot_config = {
        .oper_mode = SL_SI91X_ENTERPRISE_CLIENT_MODE,
        .coex_mode = SL_SI91X_WLAN_ONLY_MODE,
...
sl_status_t wifi_init_command_handler(console_args_t* arguments) {
    ...
    case WIFI_MODE_BLE_COEX:
        ...
        break;
    case WIFI_TRANSMIT_TEST_MODE:
        printf("Started Transmit Test mode");
        break;
    }
...
static sl_status_t show_scan_results(sl_wifi_scan_result_t* scan_result) {
    ...
    if(scan_result->scan_count) printf("\r\n   %s %24s %s", "SSID", "SECURITY", "NETWORK");
    printf("%12s %12s %s\r\n", "BSSID", "CHANNEL", "RSSI");
...
sl_status_t scan_callback_handler(
    ...
    sl_wifi_scan_result_t* scan_results_async = (sl_wifi_scan_result_t*)arg;
    if(result->scan_count > 16) {
        result->scan_count = 16;
    }
    memcpy(scan_results_async, result, (46 * result->scan_count) + 8);
...
sl_status_t wifi_scan_command_handler(console_args_t* arguments) {
    ...
    sl_wifi_interface_t interface = GET_OPTIONAL_COMMAND_ARG(
        arguments, 1, SL_WIFI_CLIENT_2_4GHZ_INTERFACE, sl_wifi_interface_t);
    ...
    sl_wifi_scan_result_t* scan_results_async = (sl_wifi_scan_result_t*)malloc((46 * 16) + 8);
    if(!synchronous) {
        sl_wifi_set_scan_callback(scan_callback_handler, scan_results_async);
    }

    status = sl_wifi_start_scan(
        interface, (ssid != NULL) ? &optional_ssid_arg : NULL, &wifi_scan_configuration);
    ...
    if(SL_STATUS_IN_PROGRESS == status) {
        ...
        if(scan_results_complete) {
            show_scan_results(scan_results_async);
        }
        free(scan_results_async);
    }
