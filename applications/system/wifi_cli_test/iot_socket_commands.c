...
sl_status_t wifi_iot_socket_bind_handler(console_args_t* arguments) {
    ...
    uint32_t ip_length =
        GET_OPTIONAL_COMMAND_ARG(arguments, 3, sizeof(sl_ipv4_address_t), uint32_t);
...
sl_status_t wifi_iot_socket_receive_from_handler(console_args_t* arguments) {
    ...
    int32_t iot_socket_status = iotSocketRecvFrom(
        sock_fd, socket_buffer, sizeof(socket_buffer), ip.bytes, &ip_length, &port);
    VERIFY_IOT_STATUS(iot_socket_status);
...
sl_status_t wifi_iot_get_host_by_name_handler(console_args_t* arguments) {
    ...
    uint32_t ip_length =
        GET_OPTIONAL_COMMAND_ARG(arguments, 2, sizeof(sl_ipv4_address_t), uint32_t);
...
sl_status_t wifi_iot_socket_send_to_handler(console_args_t* arguments) {
    ...
    uint32_t ip_length =
        GET_OPTIONAL_COMMAND_ARG(arguments, 4, sizeof(sl_ipv4_address_t), uint32_t);
    uint32_t length = strlen((char*)buffer);

    int32_t iot_socket_status =
        iotSocketSendTo(sock_fd, buffer, length, ip->bytes, ip_length, port);
    VERIFY_IOT_STATUS(iot_socket_status);
